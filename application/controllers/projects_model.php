...
public function get_project_detail($ID){
				...
				$query.= "tbl_project_posts.skills,tbl_project_posts.hourlyRate,tbl_project_posts.hrsPerWeeks,";
	...
	$client_id =$this->session->userdata('userID');
		...
		$where2 = array(
			...
			$datanotification = array(
					...
					'receiverID' 	=> $user_id,
					'msgTypes' 		=> 'awardedJob',					
					'message' 		=> 	"Congratulations! You have been awarded the job. ".$title,
					'sendDate'		=>	date("Y-m-d H:i:s")			
				);
...
public function get_propsalsByuser($filter=''){
	...
	$userID =$this->session->userdata('userID');
					...
					$query.="tbl_project_posts.ID,tbl_proposal.submitDate,tbl_proposal.paymentRequest,";
					$query.="tbl_proposal.awarded,tbl_proposal.paymentRequestDate,tbl_users.ID AS projectUserID
							FROM tbl_proposal
...
public function my_jobs_c(){
    ...
    $filter = $this->input->post('filter');
					...
					$query.= "tbl_project_posts.runingProject,";
					$query.= "tbl_project_posts.cancelProject,";
					$query.= "tbl_project_posts.archive ";
					$query.= "FROM tbl_project_posts "; 
					$query.= "INNER JOIN tbl_users ON tbl_users.ID = tbl_project_posts.userID ";
