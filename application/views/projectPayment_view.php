      ...
      <?php echo $sidebar; ?>
          <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 pull-left">
            ...
            <div class="col-lg-11" style="text-align:right;">
            	<span style="color:#000">Contract amount</span> <strong>$<?php echo number_format($projectamount,2); ?></strong>
            	 <br>
                 <p><span style="color:#000">Fund amount</span> <strong>$<?php echo number_format($projectInfo[0]['clientAmount'],2); ?></strong><br />
                 <span style="color:#000">Release Amount</span> <strong>$<?php echo number_format($projectInfo[0]['releaseamount'],2); ?></strong><br />
                 <span style="color:#000">Remaining balance</span> <strong>$<?php echo number_format(($projectInfo[0]['clientAmount']-$projectInfo[0]['releaseamount']),2); ?></strong></p>
            </div>
           		 <br><br><br><br>
            ...
            <?php if($this->session->userdata('userID')==$projectInfo[0]['projectuserID'])
			 ...
			 if($leftAmount>0)
			 	{
			 ?>
            	<a href="<?=base_url()?>projectPayment/addProjectPayment/<?=$projectID?>/<?=$receiverID?>" class="btn btn-success btn-sm pull-right">Add Fund</a>
                <?php } ?>
                <?php } ?>
            </div>
            ...
            <div class="values">
                ...
                <?php if($mileList['milestoneAmount'] == $mileList['releaseAmount']) 
					...
					<a href="<?=base_url()?>projectPayment/addProjectPayment/<?=$projectID?>/<?=$receiverID?>" class="btn btn-success">Add Fund</a>
				<?php }else
						{ ?>
