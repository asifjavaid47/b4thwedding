...
function getFreelancerRating($userID)
{
	...
	$this->db->select('tbl_feedback.*, tbl_users.fName, tbl_project_posts.title, tbl_project_posts.finishProject, tbl_project_posts.cancelProject, tbl_main_categories.cName, (SELECT sum(tbl_release_payment.amount)  FROM tbl_release_payment WHERE tbl_release_payment.projectID = tbl_project_posts.ID) as amount, (SELECT tbl_feedback.comments FROM tbl_feedback WHERE `tbl_project_posts`.`ID`=`tbl_feedback`.`projectID` AND tbl_feedback.senderID = '.$userID.') as sendercomments');
...
function getFreelancerRatingCount($userID)
{
	//$userID =$this->session->userdata('userID');
	$where = array(	
	'receiverID'	=>$userID
...
function getClientRating($userID)
{
	//$userID =$this->session->userdata('userID');
	$where = array(	
	'tbl_feedback.senderID'	=>$userID
	); 	
		
	
	$this->db->select('tbl_feedback.*, tbl_users.fName, tbl_project_posts.title, tbl_project_posts.finishProject, tbl_project_posts.cancelProject, tbl_main_categories.cName, (SELECT sum(tbl_release_payment.amount) FROM tbl_release_payment WHERE tbl_release_payment.projectID = tbl_project_posts.ID) as amount, (SELECT tbl_feedback.comments FROM tbl_feedback WHERE `tbl_project_posts`.`ID`=`tbl_feedback`.`projectID` AND tbl_feedback.receiverID = '.$userID.') as receivercomments');
	$this->db->join('tbl_users', 'tbl_users.ID = tbl_feedback.senderID','INNER');	
	$this->db->join('tbl_project_posts', 'tbl_project_posts.ID=tbl_feedback.projectID','INNER');
	$this->db->join('tbl_main_categories', 'tbl_main_categories.ID = tbl_project_posts.mainCategoryId');
	$this->db->from('tbl_feedback'); 
	$this->db->where($where);
...
function getClientRatingCount($userID)
{
	//$userID =$this->session->userdata('userID');
	$where = array(	
	'senderID'	=>$userID
...
function activeUser($userID)
{
	...
	function user_online_status($userID)
	{
		$this->db->select('is_online, userName, last_login');
		$this->db->where('ID', $userID);
		$query = $this->db->get('tbl_users');
